...
bool isLongPathEnabled() {
  ...
  bool enabled = (RegQueryValueExA(hKey, "LongPathsEnabled", NULL, NULL,
...
#ifdef _WIN32
  // First check for bundled 7z.exe (full version with RAR support)
  fs::path exeDir = getExecutableDir();
  fs::path bundled7z = exeDir / "7z.exe";
  if (fs::exists(bundled7z)) {
    return bundled7z.string();
  }

  // Fallback to system 7z.exe
  const std::string system7z = "C:\\Program Files\\7-Zip\\7z.exe";
  if (fs::exists(system7z)) {
    return system7z;
  }

  // Fallback to bundled 7za.exe (no RAR support)
  fs::path bundled7za = exeDir / "7za.exe";
  if (fs::exists(bundled7za)) {
    return bundled7za.string();
  }
...
std::pair<bool, std::string> extractArchive(const std::string &archivePath,
  ...
  if (ext != ".7z" && ext != ".zip" && ext != ".rar") {
    return {false, "Unsupported archive format: " + ext};
  }
...
#ifdef _WIN32
  // Normalize all paths to backslashes for Windows
  std::string winDestPath = destPath;
  std::string winArchivePath = archivePath;
  std::string winSevenZip = sevenZip;
  std::replace(winDestPath.begin(), winDestPath.end(), '/', '\\');
  std::replace(winArchivePath.begin(), winArchivePath.end(), '/', '\\');
  std::replace(winSevenZip.begin(), winSevenZip.end(), '/', '\\');

  // Copy archive to simple temp path to avoid cmd.exe quoting issues with
  // filenames containing spaces, dashes, and special characters
  std::string tempArchive = winDestPath + "\\a" + ext;  // e.g., C:\n\m123\a.7z

  // Ensure dest directory exists
  try {
    fs::create_directories(winDestPath);
  } catch (...) {}

  try {
    fs::copy_file(winArchivePath, tempArchive, fs::copy_options::overwrite_existing);
  } catch (const std::exception& e) {
    return {false, std::string("Failed to copy archive: ") + e.what()};
  }

  // Verify copy succeeded - check file exists and has size
  if (!fs::exists(tempArchive) || fs::file_size(tempArchive) == 0) {
    return {false, "Archive copy failed or file is empty"};
  }

  // Simple command - short paths (C:\n\m123) don't need quoting, only 7za.exe path does
  std::string errFile = winDestPath + "\\7z_err.txt";
  cmd = "\"" + winSevenZip + "\" x -y -o" + winDestPath + " " + tempArchive + " > nul 2>" + errFile;

  int result = std::system(cmd.c_str());

  if (result != 0) {
    // Try to read the actual error from 7z
    ...
    if (errStream) {
      ...
      if (errorMsg.length() > 500) {
        errorMsg = errorMsg.substr(0, 500);
      }
      // Trim whitespace
    ...
    if (errorMsg.empty()) {
      // Fall back to exit code messages
      switch (result) {
        case 1: errorMsg = "Warning: some files could not be extracted"; break;
        case 2: errorMsg = "Fatal error during extraction"; break;
        case 7: errorMsg = "Command line error"; break;
        case 8: errorMsg = "Not enough memory"; break;
        case 255: errorMsg = "User cancelled"; break;
        default: errorMsg = "7z exit code: " + std::to_string(result); break;
      }
    }
  }
  // Clean up error file
  try { fs::remove(errFile); } catch (...) {}

  // Clean up temp archive
  try { fs::remove(tempArchive); } catch (...) {}
#else
  // On Linux, we can safely capture stderr
  fs::path tempDir = fs::temp_directory_path();
  fs::path errFile = tempDir / ("nb_7z_err_" + std::to_string(getpid()) + ".txt");
  ...
  if (result != 0) {
    ...
    if (errorMsg.empty()) {
      errorMsg = "7z exit code: " + std::to_string(result);
    }
  }
...
bool installMod(const InstallTask &task) {
  ...
  try {
    // Cleanup any existing extraction with retry for file locks
    ...
    // Extract archive
    auto [extractSuccess, extractError] = extractArchive(task.archivePath, extractPath);
    if (!extractSuccess) {
      std::string errorDetail = extractError.empty() ? "Unknown error" : extractError;
      safePrint("  [" + std::to_string(task.index + 1) + "/" +
                std::to_string(task.total) + "] " + task.modName +
                " - FAILED: Extraction failed: " + errorDetail + "\n");
      g_failed++;
      return false;
    }
...
void printUsage(const char *progName) {
  ...
  std::cout << "  --temp-dir <path>      Custom temp directory for extraction (default: C:\\n or ~/.cache/nexusbridge)" << std::endl;
  std::cout << "  --threads <n>          Max threads for parallel operations (default: auto)" << std::endl;
  std::cout << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  std::string tempDir;
  if (!customTempDir.empty()) {
    // User specified custom temp dir
    tempDir = customTempDir;
    std::cout << "Using custom temp directory: " << tempDir << std::endl;
  } else {
#ifdef _WIN32
    // Use short path to avoid Windows MAX_PATH (260 char) limit
    tempDir = "C:\\n";
#else
    // Use ~/.cache/nexusbridge instead of /tmp to avoid tmpfs filling up
    const char* home = std::getenv("HOME");
    if (home) {
      tempDir = std::string(home) + "/.cache/nexusbridge";
    } else {
      tempDir = (getTempDir() / "nb_ext").string();  // Fallback to /tmp
    }
#endif
  }
  ...
  // Determine thread count (use hardware concurrency, min 4, or user-specified)
  unsigned int numThreads;
  if (maxThreads > 0) {
    numThreads = static_cast<unsigned int>(maxThreads);
    std::cout << "Using " << numThreads << " threads (user-specified)" << std::endl;
  } else {
    ...
    if (numThreads == 0)
      ...
      numThreads = 4;
    std::cout << "Using " << numThreads << " threads for parallel operations" << std::endl;
  }
...
#ifdef _WIN32
  // Check for Windows Long Path support and try to enable if disabled
  if (!isLongPathEnabled()) {
    std::cout << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << "Long Path support is DISABLED - attempting to enable..." << std::endl;

    // Create a .reg file to enable Long Path support
    std::string regFilePath = (getTempDir() / "enable_longpath.reg").string();
    std::ofstream regFile(regFilePath);
    if (regFile) {
      regFile << "Windows Registry Editor Version 5.00\r\n\r\n";
      regFile << "[HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\FileSystem]\r\n";
      regFile << "\"LongPathsEnabled\"=dword:00000001\r\n";
      regFile.close();
      ...
      // Check if it worked
      if (isLongPathEnabled()) {
        std::cout << "Long Path support ENABLED successfully!" << std::endl;
        std::cout << "NOTE: You may need to restart your computer for full effect." << std::endl;
      } else {
        std::cout << "Could not enable Long Path support (needs admin rights)." << std::endl;
        std::cout << "Please run NexusBridge as Administrator, or manually run:" << std::endl;
        std::cout << "  reg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\FileSystem\" /v LongPathsEnabled /t REG_DWORD /d 1 /f" << std::endl;
      }
    }
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
  ...
  // Calculate sizes of already-present archives
  long long existingArchiveBytes = 0;
  for (const auto& [idx, archivePath] : modArchivePaths) {
    try {
      existingArchiveBytes += static_cast<long long>(fs::file_size(archivePath));
    } catch (...) {}
  }

  // Calculate estimated install size (roughly 2x archive size for compressed archives)
  // Include both files to download AND already-present files
  long long estimatedInstallBytes = (totalDownloadBytes + existingArchiveBytes) * 2;
